...
#open file
open( $fh, "<", "Mammalian_TFBS.txt" ) || die "cannot open file";
...
while (<$fh>) {
	...
	if ( $tab2 != -1 ) {
		...
		#nothing between second and third tab > line without "from" position > skip
		if ( $tab3 != $tab2 + 1 ) {
			...
			#nothing between third and fourth tab > line without "to" position
			#compute missing "to" > 4 positions plus and minus the "from" position
			else {
				$substring_to = $substring_from + 4;
				$substring_from -= 4;
			}
...
open( $fh, ">", "distribution_trans_mamma.txt" ) || die "cannot create file";
...
$funtf->name("TRANSFAC, Säugetiere");
my @series;
push( @series, $funtf );
...
$chart->write_output("distribution_trans_mammal.pdf");
